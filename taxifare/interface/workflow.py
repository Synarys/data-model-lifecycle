...
def transition_model(current_stage: str, new_stage: str):
    mlflow_transition_model(current_stage, new_stage)
...
def notify(old_mae, new_mae):
    ...
    channel = '2007' # Change to your batch number
    url = f"{base_url}/{channel}/messages"
    author = 'Synarys' # Change this to your github nickname
...
def train_flow():
    ...
    preprocess = preprocess_new_data.submit(min_date, max_date)
    evaluate = evaluate_production_model.submit(min_date, max_date, wait_for=[preprocess])
    train = re_train.submit(min_date, max_date, 0.2, wait_for=[preprocess])

    old_mae = evaluate.result()
    new_mae = train.result()

    if new_mae < old_mae and new_mae < 2.5:
        transition = transition_model.submit('Staging', 'Production', wait_for=[evaluate, train])
        
    notif = notify.submit(old_mae, new_mae, wait_for=[evaluate, train])
